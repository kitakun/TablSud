...
{
    [Obsolete("NancyContext.CurrentUser always null")]
    public class UserTokenViewEngineMatcher : ISuperSimpleViewEngineMatcher
    {
        /// <summary>
        /// Compiled Regex for accessing user object
        /// </summary>
        private static readonly Regex UserSubstitutionsRegEx;

        static UserTokenViewEngineMatcher()
        {
            // This regex will match strings like:
            // @User.Name
            UserSubstitutionsRegEx = new Regex(@"@User\.(?<TranslationKey>[a-zA-Z0-9-_.()]+);?", RegexOptions.Compiled);
        }

        public string Invoke(string content, dynamic model, IViewEngineHost host)
        {
            MatchCollection reslt = UserSubstitutionsRegEx.Matches(content);
            if (reslt.Count > 0)
            {
                ...
                foreach (Match match in reslt)
                {
                    ...
                    var windowsUser = (IPrincipal)owinEnv["owin.RequestUser"];
